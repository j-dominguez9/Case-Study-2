...
```{r Format data set to prepare for analysis}

### identify and transform factor variables that are not currently classified as factors
...
### an RMSE of 7897.961 is too high for what we're looking for, let's consider alternative models

```


```{r Forward selection - No Salary}
ols_step_forward_p(mi_lm_2, penter = 0.01, details = TRUE)
...
```{r Stepwise selection - No Salary}
...
```{r Comparing models}
### By using Mallow's Cp, adjusted r^2, AIC, and PRESS in conjunction as metrics, we can feel confident in our model selection of best fit.
